...
def add_item_to_cart_api(item_details:CartItemCreate, user_data: Annotated[dict, Depends(authorization)]):
    cart_info = add_item_to_cart(item_details.model_dump(),user_data.get("id"))
    return cart_info

@cart_router.patch("/cart/upadate_item",status_code=200)
def update_item_to_cart_api(item_details:CartItemUpdate, user_data: Annotated[dict, Depends(authorization)]):
    cart_info = update_cart_item(item_details.model_dump(),user_data.get("id"))
    return cart_info

@cart_router.delete("/cart/delete_item",status_code=403)
def delete_item_to_cart_api(item_details:CartItemDelete, user_data: Annotated[dict, Depends(authorization)]):
