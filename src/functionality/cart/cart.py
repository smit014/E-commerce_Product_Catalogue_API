...
def add_item_to_cart(item_data,user_id):
    try:
        ...
        db.commit()
        return JSONResponse({"message": "Item added to cart successfully","id" :str(cart.id)})
...
def update_cart_item( item_data,user_id):
    ...
    if cart:
        if cart.user_id == user_id:
            ...
            raise HTTPException(status_code=403, detail="you can'taccessed this cart")
    else:
...
def delete_cart_item(item_data,user_id):
    ...
    cart_id = item_data.get("cart_id")
    cart = db.query(Cart).filter_by(id=cart_id,status ="active",user_id=user_id).first()
    if cart:
        if cart.user_id == user_id:
            cart_item = db.query(CartItem).filter_by(id=item_id, cart_id=cart_id).first()
            if cart_item:
