...
def create_review(review_details, user_details):
    try:
        ...
        if not review_data:
            ...
            db.commit()
            return JSONResponse(
                {"Message": "Review created successfully", "Review_id ": str(id)}
            )
        else:
            raise HTTPException(
                status_code=403,
                detail="you already have a review so you can't create a new review",
            )
    except Exception:
        raise HTTPException(status_code=400, detail="Enter the mandatory feilds")
    ...
    if review_data:
        if review_data.user_id == user_id:
            review_data.content = (
                ...
                if review_details.get("content") is not None
                else review_data.content
            )
            review_data.rating = (
                review_details.get("rating")
                if review_details.get("rating") is not None
                else review_data.rating
            )
            ...
            raise HTTPException(
                status_code=401, detail="You have not rights to update this Review"
            )
    else:
        raise HTTPException(status_code=404, detail="Review not found")
...
def delete_review(review_id, user_id):
    ...
    if review_data:
        if review_data.user_id == user_id:
            ...
            raise HTTPException(status_code=401, detail="you cannot delete this review")
    else:
        raise HTTPException(status_code=404, detail="Review not found")
